...
public:
      ...
      , right(nullptr)
    {
        ...
        if (depth == 0 || (ivals.size() < minbucket && ivals.size() < maxbucket)) {
            ...
            for (const auto& item : intervals)
                sorted_t_start.push_back(item.start);
            ...
            std::sort(intervals.begin(), intervals.end(), IntervalStartCmp());
            for (const auto& item : intervals)
                sorted_t_start.push_back(item.start);
    ...
    // Call f to count all intervals near the range [start, stop]:
    template <class UnaryFunction>
    void count_near(const Scalar& start, const Scalar& stop, UnaryFunction g) const {
        if (!intervals.empty() && ! (stop < intervals.front().start)) {
            g(sorted_t_start);
        }
        if (left && start <= center) {
            left->count_near(start, stop, g);
        }
        if (right && stop >= center) {
            right->count_near(start, stop, g);
        }
    }
    ...
    // Call f to count all intervals overlapping [start, stop]
    template <class UnaryFunction>
    void count_overlapping(const Scalar& start, const Scalar& stop, UnaryFunction g) const {
        auto filterF = [&](const std::vector<Scalar>& sorted_endpoints) {
            auto loc = std::lower_bound(sorted_endpoints.begin(), sorted_endpoints.end(), stop);
            g(loc - sorted_endpoints.begin());
        };
        count_near(start, stop, filterF);
    }
    ...
    void visit_overlapping_with_durability(const Scalar& start, const Scalar& stop, int durability, UnaryFunction f) const {
        ...
        auto filterF = [&](const interval& interval) {
            if (interval.stop >= start && interval.start <= stop) {
                ...
                int left = MAX(interval.start, start);
                int right = MIN(interval.stop, stop);
    ...
    int countOverlapping(const Scalar& start, const Scalar& stop) const {
        int result_count = 0;
        count_overlapping(start, stop,
                          [&](const int& count) { 
                            result_count += count; 
                          });
        return result_count;
    ...
    std::vector<Scalar> sorted_t_start;
private:
